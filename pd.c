/*
pd.c v5.2.0
Copyright 1991-2022 (c) Scott L. Fehrman
...
Scott Fehrman, sfehrman@me.com

Compile options:
Linux: gcc -o pd pd.c -O3 [ -lcurses | -lncurses ]
MacOS: gcc -o pd pd.c -O3 -lncurses
*/
...
#include <string.h>               /* string and memory handling */
#include <getopt.h>               /* command line option processing */
...
#define       VERSION  "5.2.0"    /* program revision stamp */
...
int           
   ...
   iFlagSortName = TRUE,           /* flag, sort ny name */
...
struct stat   
   *pStat, 
   *pStatLink;             /* ptr to file status struct */
...
void command_date_option(char *pOpt)
{
...
void command_no_option(char *pOpt)
{
...
void command_sort_option(char *pOpt)
{
...
void command_unit_option(char *pOpt)
{
...
void command_type_option(char *pOpt)
{
...
void store_entry(int iType, char *pszName, struct stat *pMyStat, int iLink, 
...
void read_directory(DIR *pDir)
{

   /* get the name of all the entries in a directory */

   int  iTemp, iDone = FALSE;
...
int read_entry(char *szFullName, char *szEntryName, int iCommandLine)
{
...
void display_file()
{
   ...
   register int t, i;
   int iColCnt, iColWidth, iNblanks;
...
void display_alternate(EntryData *pEntry, char *szPathName)
{
   int  iSpace = 3;
...
void display_expanded(EntryData *pEntry, char *szPathName) 
{
   ...
   if ( iFlagInode )   (void)printf("%12llu   ",pEntry->Stat.st_ino);
   if ( iFlagLinkCnt ) (void)printf("%6d    ",pEntry->Stat.st_nlink);
   if ( iFlagBlocks )  (void)printf("%6lld  ",pEntry->Stat.st_blocks);
   if ( iFlagSize )
   {
...
void output_alt_header()
{
   ...
   if ( iFlagAcls )     (void)printf("Access Control Lists [mask] (effective) ");
...
void output_timedate(struct tm *pTm)
{
...
static int compare(EntryData *e1, EntryData *e2)
{
...
void align_column(int iLong, int iCurrent, int iGap)
{
   register int i, t;
   t = iLong - iCurrent;
...
void program_help()
{
   ...
   (void)printf("           [none|name|dtm|dta|dts|user|group|inode|link|size|type,rev]\n");
